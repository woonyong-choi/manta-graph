...
| Edge geometry | passed | Direct and preview SVG endpoints use each dot's measured viewport centre, converted back through the active graph scale. Nested visuals, padding, wrapped labels, and hover transforms cannot offset a line from its dot. |
