...
3. Only resolved, non-embedded outgoing wikilinks are shown. Dot-prefixed internal roots, private/source, generated/archive, and retired/superseded destinations are excluded by default from routes and parent navigation; users may configure folder/status exclusions. Hidden internal roots remain excluded. Ordinary root folders such as `brain` and `inbox` are never assigned special meaning.
...
12. Display mode and explicit exclusions are saved as preferences. Search, group collapse, pan, zoom, hover preview, and node positions remain ephemeral.
13. No plugin action writes Markdown, Canvas, sidecars, maps, relationships, or layout state. Only normalized display/exclusion preferences are stored in plugin data.json.
...
Obsidian's public plugin API does not expose the live fold ranges of the active Markdown editor. Manta Graph therefore does not inspect internal editor DOM or pretend to mirror folds. Outline disclosure remains session-only and does not hide authored links from the current-note graph.
